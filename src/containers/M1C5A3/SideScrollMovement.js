...
import Sprite from '../../components/Sprites/Sprite4';
...
const SideScrollMovement = ({
	...
	const obsSprites = JSON.parse(JSON.stringify(data.obsSprites));
			...
			timeOut = setTimeout(
				() => {
					if (typeof steps[step].indexExpresion == 'number') {
						setIndex(steps[step].indexExpresion);
					}
					...
					if (!steps[step].stop || start) {
						setStep(step + 1);
						if (typeof steps[step].showObject == 'string') {
							setShowObject(steps[step].showObject);
						}
					}
	...
	useFrame(() => {
		...
		if (spriteRef.current && camera.position.x != spriteRef.current.position.x) {
			camera.position.set(
				...
				steps[0].toY +
					2.5 +
					Math.abs(steps[0].toY - spriteRef.current.position.y) / 3,
				camera.position.z,
			);
